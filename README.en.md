...
This is the aggregate AgentDock plugin repository for the EasyEDA AI toolchain. The current plugin version is **2.8.7**. The parent repository stores only plugin manifests, the MCP launch configuration, build scripts, and pinned component commits. Five Skills and one PCB MCP are maintained in separate private GitHub repositories and assembled through Git submodules.
...
| `skills/easyeda-pcb-layout-routing` | 5.3.1 | [`easyeda-skill-pcb-layout-routing`](https://github.com/A-m-o-r-F-a-t-i/easyeda-skill-pcb-layout-routing) | Board outlines, placement, routing, pours, silkscreen, and release criteria; ordinary components remain unlocked and visible reference-designator silkscreen is removed while Designator identity is preserved |
...
| `mcp/easyeda-pcb` | 2.4.8 | [`easyeda-mcp-pcb`](https://github.com/A-m-o-r-F-a-t-i/easyeda-mcp-pcb) | Twenty-one typed, guarded, and independently verified production operations; reference-designator cleanup hides Designator silkscreen while preserving the mandatory identity attribute |
...
```text
dist/easyeda-plugin-2.8.7/
dist/easyeda-plugin-2.8.7.zip
```
