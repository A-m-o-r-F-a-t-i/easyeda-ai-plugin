...
This is the aggregate AgentDock plugin repository for the EasyEDA AI toolchain. The current plugin version is **2.9.1**. The parent repository stores only plugin manifests, the MCP launch configuration, build scripts, and pinned component commits. Five Skills and one PCB MCP are maintained in separate public GitHub repositories and assembled through Git submodules.
...
| `skills/easyeda-pcb-layout-routing` | 5.5.1 | [`easyeda-skill-pcb-layout-routing`](https://github.com/A-m-o-r-F-a-t-i/easyeda-skill-pcb-layout-routing) | Board outlines, placement, routing, pours, silkscreen, and release criteria; small low-component boards may use one complete placement round, while larger boards are partitioned and cannot advance until the current overlap gate passes |
...
```text
dist/easyeda-plugin-2.9.1/
dist/easyeda-plugin-2.9.1.zip
```
