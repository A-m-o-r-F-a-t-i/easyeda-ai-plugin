...
| 成员 | 版本与职责 |
| --- | --- |
| PCB MCP | 3.0.1：默认21工具，所有常规坐标默认 mil，直接大批量编辑、完整元件网络朝向读取、整体/局部 SVG |
| PCB Skill | 6.0.1：MCP 优先、mil 默认、设计经验、按需分析，无强制准备链或设计门控 |
| API Skill | 2.4.1：MCP 3.0.1/mil 默认路由、原生能力资料、Bridge、扩展开发和明确封装缺口诊断 |
| 其他三个 Skill | 保持现有版本，原理图和格式能力不受本次改造影响 |

普通编辑使用位号、U1.12 端点和层名，编辑坐标、总览/引脚映射、拾取、SVG区域及贴片坐标均默认 mil；需要毫米时显式填写 mm。未填写字段保持原值。MCP 内部处理对象和参数转换、批量分片、原生返回与部分成功。没有自动布局、寻路或隐藏几何优化。检查工具返回事实，不决定编辑权限。
...
构建输出 dist/easyeda-plugin-3.0.1.zip。构建使用子模块的已提交 HEAD，不会把未提交源文件误当发布内容。源码回滚不会恢复已修改的 PCB。
