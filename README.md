...
这是 AgentDock 的嘉立创 EDA AI 插件聚合仓库，当前插件版本为 **2.10.0**。仓库本身只保存插件清单、MCP 启动配置、构建脚本和固定的组件提交；5 个 Skill 与 1 个 PCB MCP 均由独立的 GitHub 公开仓库维护，并通过 Git submodule 组合成完整插件。
...
| `skills/easyeda-pcb-layout-routing` | 5.6.0 | [`easyeda-skill-pcb-layout-routing`](https://github.com/A-m-o-r-F-a-t-i/easyeda-skill-pcb-layout-routing) | PCB 板框、布局、布线、铺铜、丝印与生产验收；圆形以原点为圆心，多边形以原点顶点锚定，长方形可让相邻两边落在 X/Y 轴上 |
...
| `mcp/easyeda-pcb` | 2.6.0 | [`easyeda-mcp-pcb`](https://github.com/A-m-o-r-F-a-t-i/easyeda-mcp-pcb) | 对当前 EasyEDA PCB 文档执行 21 个受保护生产操作；新板框使用圆心或多边形顶点原点锚定，并在元件、独立焊盘和过孔写入前后阻止跨对象焊盘重叠 |
